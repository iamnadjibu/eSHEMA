...
import { UserCheck, ShieldCheck, Clock, CheckCircle2, XCircle, AlertCircle, RefreshCw, Ban, UserCog } from 'lucide-react';
import { getAllUsers, approveUserStatus, updateUserStatusAndRole } from '../../firebase/authService';
...
export default function UserApprovalView({ currentUser }) {
  ...
  const handleUpdateStatusAndRole = async (uid, newStatus, newRole) => {
    setActionMessage('');
    try {
      await updateUserStatusAndRole(uid, newStatus, newRole, currentUser?.email || 'admin@ksp.rw');
                  ...
                  <tr key={user.uid} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="p-4">
                      {user.status === 'suspended' ? (
                        <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-red-500/20 text-red-400 border border-red-500/30 uppercase flex items-center gap-1 w-fit">
                    ...
                    <td className="p-4 text-right">
                      {user.role === 'super_admin' ? (
                        ...
                        <div className="flex items-center justify-end gap-2">
                          <select 
                            value={user.role} 
                            onChange={(e) => handleUpdateStatusAndRole(user.uid, user.status, e.target.value)}
                            className="bg-slate-800 text-xs text-white border border-slate-700 rounded-lg px-2 py-1 outline-none focus:border-blue-500"
                          >
                          ...
                          {user.status === 'suspended' ? (
                            <button 
                              onClick={() => handleUpdateStatusAndRole(user.uid, 'approved', user.role)}
                              className="px-2.5 py-1.5 bg-emerald-950/60 hover:bg-emerald-900/80 text-emerald-300 text-[11px] font-bold rounded-xl border border-emerald-800 transition flex items-center gap-1"
                            >
                              <CheckCircle2 className="w-3.5 h-3.5" /> Restore
                            </button>
                          ) : (
                            <button 
                              onClick={() => handleUpdateStatusAndRole(user.uid, 'suspended', user.role)}
                              className="px-2.5 py-1.5 bg-red-950/60 hover:bg-red-900/80 text-red-300 text-[11px] font-bold rounded-xl border border-red-800 transition flex items-center gap-1"
                            >
